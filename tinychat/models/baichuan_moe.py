...
# from .configuration_baichuan import BaiChuanMoEConfig
...
class MixtralMLP(nn.Module):
    ...
    def __init__(self, hidden_size, intermediate_size, hidden_act, moe_experts_routed, moe_experts_selected, moe_experts_fixed):
        super().__init__()
        ...
        self.router = torch.nn.Linear(hidden_size, self.num_experts_routed, bias=False)

        self.local_experts_routed = torch.nn.ModuleList()
        for i in range(self.num_local_experts_routed):
            self.local_experts_routed.append(MLP(hidden_size, intermediate_size, hidden_act))

        if moe_experts_fixed >= 1:
            self.local_experts_fixed = MLP(hidden_size, intermediate_size*moe_experts_fixed, hidden_act)
        else:
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        final_hidden_states = torch.zeros(
            (s * b, h), dtype=hidden_states.dtype, device=hidden_states.device
        )

        # One hot encode the selected experts to create an expert mask
        # this will be used to easily index which expert is going to be sollicitated
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts_routed).permute(2, 1, 0)

        # Loop over all available experts in the model and perform the computation on each expert
        for expert_idx in range(self.num_experts_routed):
            expert_layer = self.local_experts_routed[expert_idx]
            ...
            # current_state = hidden_states[None, top_x_list].reshape(-1, h)
            current_state = hidden_states[None, top_x_list].permute(1, 0, 2)
            current_output = expert_layer(current_state)
            current_hidden_states = routing_weights[top_x_list, idx_list, None] * current_output.view(-1, h)

            # However `index_add_` only support torch tensors for indexing so we'll use
            # the `top_x` tensor here.
            final_hidden_states.index_add_(0, top_x, current_hidden_states.to(hidden_states.dtype))
        final_hidden_states = final_hidden_states.reshape(s, b, h)

        if self.local_experts_fixed:
            final_hidden_states += self.local_experts_fixed(hidden_states).reshape(s, b, h)
...
class Attention(nn.Module):
    ...
    def __init__(self, config: BaiChuanMoEConfig):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = RotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings, base=self.rope_base)
    ...
    def forward(
        ...
        # @menxin : flash attn is all you need
...
class DecoderLayer(nn.Module):
    def __init__(self, config: BaiChuanMoEConfig):
        ...
        if config.moe_experts_routed == 1:
            self.mlp = MLP(hidden_size=config.hidden_size, intermediate_size=config.intermediate_size, hidden_act=config.hidden_act)
        # MoE
        else:
            self.mlp = MixtralMLP(config.hidden_size, config.intermediate_size, config.hidden_act,
                                  config.moe_experts_routed, config.moe_experts_selected, config.moe_experts_fixed)
...
class PreTrainedModel(PreTrainedModel):
    config_class = BaiChuanMoEConfig
    ...
    def __init__(self, config: BaiChuanMoEConfig):
        ...
        if config.num_experts_routed_per_layer:
            ...
            for index in range(config.num_hidden_layers):
                ...
                layers.append(DecoderLayer(config_))
...
class BaiChuanMoEForCausalLM(PreTrainedModel):
